...
import { Headings } from "@/app/(main)/insights-articles/[slug]/utils";
...
export const TableOfContents = ({ headings }: TableOfContentsProps) => {
  ...
  return (
    <aside className="flex-col gap-5 lg:flex">
      <span className="text-foreground text-sm font-bold">On this page</span>
      <nav className="flex flex-col gap-3">
        {Object.keys(headings ?? []).map((item) => (
          <a
            ...
            className={clsx(
              "relative w-fit inline-block text-sm text-muted-foreground transition-colors cursor-pointer hover:text-foreground group"
            )}
          >
            {headings[item].title}
            <span
